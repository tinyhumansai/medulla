...
## How a dispatched task runs one

Two additions to the task protocol the local hub speaks to its workers, both
optional:
...
- A task frame may carry a `workflow` field. Naming one makes the worker run that
  ...
  work-snapshot attachment are all the ordinary ones, so a dispatcher that
...
Nothing about a workflow leaves the machine. The store, the runs, the copilot
turn, and the MCP tools are all served locally; the backend is never asked about
a graph and never told one exists.
