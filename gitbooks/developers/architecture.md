...
* [`src/sdk/`](../../src/sdk/) — the `medulla` SDK crate: a **UI-free logic library**. The backend HTTP/SSE client, the runtime adapters over the embedded core, sessions, workflows, and the tiny.place integration. Reusable from any Rust program.
...
Because the UI depends only on the `Runtime` trait and the client speaks a small set of HTTP/SSE surfaces, the whole system can be exercised offline. The test suites spin up in-process stand-ins — a mock HTTP/SSE backend, a mock tiny.place API server, and mock `claude`/`codex`/`opencode` CLIs that emit realistic provider stream-JSONL — all under `src/sdk/tests/support/`. Tests stay deterministic and need no live network. See [Contributing](contributing.md) for how to run them.
