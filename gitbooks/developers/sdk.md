...
`medulla` is the library crate at [`src/sdk/`](../../src/sdk/): a UI-free logic
...
| --- | --- | --- |
| `workflows` | on | The [`flow_engine`](../../src/sdk/src/flow_engine/) adapter seam onto the vendored `tinyflows` engine, the [`workflows`](../../src/sdk/src/workflows/) module (definitions, store, runs, authoring), and the [`mcp`](../../src/sdk/src/mcp/) module. Kept behind a feature so a slim build can drop the engine and its jq expression stack; on by default because the shipped TUI exposes workflows. |
...
[`medulla::runtime::Runtime`](../../src/sdk/src/runtime/mod.rs), plus its
...
[`src/sdk/src/runtime/`](../../src/sdk/src/runtime/):
...
[`medulla::client::MedullaClient`](../../src/sdk/src/client/) is a typed surface
...
[`src/sdk/examples/`](../../src/sdk/examples/) holds runnable examples that
...
| --- | --- |
| [`agents/`](../../src/sdk/src/agents/) | Where agent templates come from: the built-in coding catalog, the on-disk `.medulla/agents/*.toml` store that supersedes it, and the installer between them. |
| [`attribution/`](../../src/sdk/src/attribution/) | Git commit attribution: the `Co-authored-by` trailer and the hook shims that carry it without disabling a repository's own hooks. |
| [`auth/`](../../src/sdk/src/auth/) | An RFC 8252 loopback OAuth flow against the backend, plus the pure URL and query helpers the CLI and tests share. |
| [`bridge/`](../../src/sdk/src/bridge/) | Message delivery bridges for local and remote agent communication. |
| [`client/`](../../src/sdk/src/client/) | The HTTP and SSE client for the orchestration backend. |
| [`clipboard/`](../../src/sdk/src/clipboard/) | Clipboard writers: a platform binary first, then OSC 52. See [Troubleshooting](troubleshooting.md#copying-out-of-medulla). |
| [`codex_app_server/`](../../src/sdk/src/codex_app_server/) | A pooled client for `codex app-server`. See [Harness integration](harness-integration.md#codex-on-a-shared-process). |
| [`codex_overrides/`](../../src/sdk/src/codex_overrides/) | Codex `-c` config overrides that make a routed Codex run reach a non-OpenAI model. |
| [`config/`](../../src/sdk/src/config/) | The `medulla.tui.json`-compatible config the TUI reads, plus the `backend` section. Permissive: missing fields take defaults, unknown fields are ignored. |
| [`control_socket/`](../../src/sdk/src/control_socket/) | The local control socket a spawned harness reaches, and the grant tokens that scope it. |
| [`core_host/`](../../src/sdk/src/core_host/) | Booting the embedded OpenHuman core in this process. |
| [`daemon/`](../../src/sdk/src/daemon/) | The headless `medulla daemon`: offering this machine's coding-agent CLIs as an addressable agent, over plain prompts and the `medulla-task/1` protocol. |
| [`flow_engine/`](../../src/sdk/src/flow_engine/) | The adapter seam between Medulla and the `tinyflows` workflow engine (`workflows` feature). |
| [`harness_contract/`](../../src/sdk/src/harness_contract/) | The public agent-harness wire-contract types. See [Harness integration](harness-integration.md#the-wire-contract). |
| [`harness_hooks/`](../../src/sdk/src/harness_hooks/) | The hooks Medulla installs into a launched harness, and the launch policy around them. |
| [`harness_work/`](../../src/sdk/src/harness_work/) | What a coding-agent harness is working on, in one vocabulary. |
| [`history_upload/`](../../src/sdk/src/history_upload/) | Sharing local coding-agent history to earn onboarding credit. |
| [`home/`](../../src/sdk/src/home/) | The Medulla home directory and the early `.env` loader. |
| [`hub/`](../../src/sdk/src/hub/) | The task-sender hub: the outbound half of the harness plane. |
| [`inference_proxy/`](../../src/sdk/src/inference_proxy/) | The loopback attribution proxy. See [Attribution and routing](attribution-and-routing.md). |
| [`init/`](../../src/sdk/src/init/) | Workspace initialisation: registering a directory and authoring its `MEDULLA.md`. |
| [`logging/`](../../src/sdk/src/logging/) | The one line-sink type every subsystem narrates through. |
| [`mcp/`](../../src/sdk/src/mcp/) | Medulla's own MCP server, offered to the harnesses it spawns (`workflows` feature). |
| [`onboarding/`](../../src/sdk/src/onboarding/) | First-run worker registration orchestration. |
| [`protocol/`](../../src/sdk/src/protocol/) | Medulla's own wire protocol for the TUI and daemon, plus the centralized environment-variable resolution both share. |
| [`runtime/`](../../src/sdk/src/runtime/) | The `Runtime` trait, its snapshot contract, and the `openhuman` and `mock` implementations. |
| [`session_history/`](../../src/sdk/src/session_history/) | Recent-session history for local harness sessions. |
| [`sessions/`](../../src/sdk/src/sessions/) | Interactive coding-agent session management: the two lifetime classes, the two turn-source drivers, and the machinery that runs them. |
| [`ui/`](../../src/sdk/src/ui/) | The UI-facing data surface: `events`, `agents` lane folding, `stream` derivations, `chat_store`, the `work` panel, and `util`. Rendering lives in `medulla-tui`. |
| [`update/`](../../src/sdk/src/update/) | Release update checking and self-update. |
| [`worker_profile/`](../../src/sdk/src/worker_profile/) | The persisted first-run worker profile. |
| [`workflows/`](../../src/sdk/src/workflows/) | Authored, durable, multi-step work: workflow definitions and their runs (`workflows` feature). |
| [`wrapper/`](../../src/sdk/src/wrapper/) | The transparent harness wrapper behind `medulla codex`, `medulla claude`, and `medulla opencode`. |
