...
/// The default Medulla home directory used as the memory workspace parent
/// (`<home>/memory`) when neither the environment nor config sets a workspace.
/// Resolves to `~/.medulla`, falling back to `.medulla` when no home is known.
pub fn default_medulla_home() -> std::path::PathBuf {
    dirs::home_dir()
        .map(|h| h.join(".medulla"))
        .unwrap_or_else(|| std::path::PathBuf::from(".medulla"))
}
