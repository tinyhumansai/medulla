...
- [`prepare_commit_msg.rs`](./prepare_commit_msg.rs) — Generate the git hook directory that adds the `Co-authored-by` trailer from the `MEDULLA_ATTRIBUTION` environment variable. This is the mechanism of record for *every* provider: a harness CLI's own attribution setting (where one exists) only asks the model to write the trailer, so it drops out whenever the task brief dictates a commit message. Because `core.hooksPath` redirects *every* hook, the directory shims every client-side hook name and delegates to the repository's own hook of that name; the directory is process-wide so concurrent harnesses cannot delete each other's.
