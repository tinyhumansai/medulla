...
//!
//! Both are deliberately testable without a core: [`super::progress`] is a pure
//! function over an enum the vendored crate already defines, and
...
/// The single mapped event a progress variant folds into.
fn only_event(progress: &AgentProgress) -> (String, serde_json::Value) {
    let mut mapped = semantic_events(progress);
...
fn turn_and_iteration_boundaries_fold_to_status() {
    let (kind, payload) = only_event(&AgentProgress::TurnStarted);
    assert_eq!(kind, "status");
    assert_eq!(payload["state"], "running");
    assert_eq!(payload["detail"], "turn started");

    let (kind, payload) = only_event(&AgentProgress::IterationStarted {
        iteration: 3,
        max_iterations: 40,
    });
    assert_eq!(kind, "status");
    ...
    let (kind, payload) = only_event(&AgentProgress::TurnCompleted { iterations: 7 });
    assert_eq!(kind, "status");
...
#[test]
fn text_and_thinking_deltas_fold_to_their_text_kinds() {
    let (kind, payload) = only_event(&AgentProgress::TextDelta {
    ...
    let (kind, payload) = only_event(&AgentProgress::ThinkingDelta {
        delta: "hmm".to_string(),
        iteration: 1,
    });
    assert_eq!(kind, "agent_thinking");
    assert_eq!(payload["text"], "hmm");
}

#[test]
fn tool_call_started_folds_to_a_tool_call_carrying_its_arguments() {
    let (kind, payload) = only_event(&AgentProgress::ToolCallStarted {
...
fn a_tool_call_without_a_label_displays_its_tool_name() {
    let (_, payload) = only_event(&AgentProgress::ToolCallStarted {
...
fn a_failed_tool_call_folds_to_an_error_flagged_tool_result() {
    let (kind, payload) = only_event(&AgentProgress::ToolCallCompleted {
        call_id: "call-1".to_string(),
        tool_name: "bash".to_string(),
        success: false,
        output_chars: 12,
    ...
    assert_eq!(payload["output_bytes"], 12);
...
fn accounting_only_progress_folds_to_nothing() {
    let mapped = semantic_events(&AgentProgress::TurnCostUpdated {
...
async fn events_queued_when_the_call_resolves_are_drained() {
    ...
    // completes with both events still sitting in the channel.
    let call = async move {
    ...
    let text: String = log
        .iter()
        .map(|(_, payload)| payload["text"].as_str().unwrap_or_default())
        .collect();
