...
async fn running_a_workflow_that_is_disabled_is_refused_rather_than_silently_skipped() {
    ...
    // values: `local::LocalRun::start` checks `workflow.enabled` *before* it ever
...
// Run-projection and waiting cases (`workflow_run_get`/`workflow_runs`
// reading back a settled run) live in the sibling `runs` module — this file
// was at the repository's 500-line ceiling.
