...
pub fn collect_expressions(value: &Value) -> Vec<(String, String)> {
    fn walk(value: &Value, location: &str, out: &mut Vec<(String, String)>) {
        match value {
            ...
            Value::String(text) if tinyflows::expr::is_expression(text) => {
...
pub fn parse_node_binding(expr: &str) -> Option<NodeBinding> {
    ...
    let field_path = captures.get(3)?.as_str().trim_end_matches('.').to_string();
    if field_path.is_empty() {
        return None;
    }
    Some(NodeBinding {
        node_id: captures.get(1)?.as_str().to_string(),
        through_envelope: captures.get(2).is_some(),
        field_path,
    })
}
...
pub fn node_of<'a>(graph: &'a WorkflowGraph, id: &str) -> Option<&'a tinyflows::model::Node> {
