...
// ---- propose mode ----
//
// The boundary that keeps a review turn from editing a graph. It is enforced
// here rather than in the prompt because a standing instruction is a request a
// confused turn can talk itself past, and a tool that is not served is not.
...
fn propose_mode_withholds_every_verb_that_writes_a_graph() {
    for withheld in [
        ...
        assert!(
            !ToolMode::Propose.allows(withheld),
            "{withheld} must not reach a review turn"
        );
        assert!(
            ToolMode::Full.allows(withheld),
            "{withheld} is still an authoring verb"
        );
    }
}
...
async fn a_review_turn_calling_a_withheld_tool_is_refused_and_told_what_to_use() {
    ...
    // A refusal that only says "no" costs a round trip on an agent working out
    // whether it misread the tool list.
...
fn the_mode_is_read_from_the_environment_and_defaults_to_full() {
    ...
    // An unrecognised value must not leave a turn with no tools and no way to
    // say why, so it falls back to the full surface rather than erroring.
    assert_eq!(
        ToolMode::from_env(&HashMap::from([(
            TOOL_MODE_ENV.to_string(),
            "nonsense".to_string()
