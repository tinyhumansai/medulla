...
//!
//! Adapted from the sibling `openhuman` host's dry-run diagnostics.
...
fn null_binding(
    graph: &WorkflowGraph,
    node_id: &str,
    null: &tinyflows::expr::NullResolution,
) -> NullBinding {
    let reads_from = super::super::gates::parse_node_binding(&null.expression)
