...
use super::*;
use crate::workflows::types::{ProposalStatus, ProposalVerification};
use serde_json::json;
...
fn an_unknown_proposal_is_none_rather_than_an_error() {
    let home = dir();
    assert!(read(home.path(), "no-such-proposal")
        .expect("read")
        .is_none());
}
...
fn fingerprints_distinguish_graphs_and_agree_with_themselves() {
    use crate::workflows::types::fingerprint;

    let graph: tinyflows::model::WorkflowGraph = serde_json::from_value(json!({
        "nodes": [{ "id": "t", "kind": "trigger", "name": "start" }],
        "edges": []
    }))
    .expect("the fixture graph should parse");
    let changed: tinyflows::model::WorkflowGraph = serde_json::from_value(json!({
