...
async fn a_chunked_pinned_request_is_not_buffered_past_the_rewrite_limit() {
    ...
    // over the limit always spans three frames no matter where the limit sits.
    // A fixed 1 MiB chunk would silently collapse to a single frame if
    // `MAX_REWRITE_BYTES` ever dropped below it, unexercising the very branch
