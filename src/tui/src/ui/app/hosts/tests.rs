...
/// Press `n` on whatever the cursor is on.
fn press_new_agent(app: &mut App) {
    let _ = app.on_event(crossterm::event::Event::Key(
        crossterm::event::KeyEvent::new(
...
#[test]
fn a_local_host_declares_its_agent_without_leaving_the_page() {
    // Roles are assigned here and nowhere else, so the flow has to run here:
    // it used to switch to the Agents tab, which meant deciding what an agent
    // is for on a page that cannot show its toggles.
    let (mut app, _dir) = app_with(vec![worker("medulla-claude", "this-device")], Vec::new());
    app.set_local_sessions(crate::ui::app::rail::tests::shell_harnesses(
        crate::worker::pty::PtyManager::new(),
    ));
    // Captured after the navigation, which is what puts the app on Hosts.
    cursor_to(&mut app, 0);
    let tab = app.tab().to_string();
    assert_eq!(tab, "Hosts");
    assert!(
        app.hosts_cursor_on_host(),
        "the cursor is on the local host"
    );

    press_new_agent(&mut app);

    assert!(
        app.agent_picker_open(),
        "the create flow is on screen: {}",
        app.status()
    );
    assert_eq!(app.tab(), tab, "and it did not move the operator off Hosts");
}

#[test]
fn a_remote_host_declares_its_own_agents() {
    // The v1 capability split: this end is a viewer of another machine's fleet.
    let (mut app, _dir) = app_with(vec![worker("peer", "7Kx")], Vec::new());
    app.set_local_sessions(crate::ui::app::rail::tests::shell_harnesses(
        crate::worker::pty::PtyManager::new(),
    ));
    // Row 1 is the remote host header.
    cursor_to(&mut app, 1);
    assert!(!app.selected_host_is_local());

    press_new_agent(&mut app);

    assert!(!app.agent_picker_open(), "no create flow for a remote host");
    assert!(
