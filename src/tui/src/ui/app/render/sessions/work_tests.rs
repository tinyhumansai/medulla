...
fn draw(work: &WorkSnapshot, width: u16, height: u16) -> String {
    ...
    terminal
        .draw(|f| {
            app.draw_sessions_work(f, Rect::new(0, 0, width, height), work);
        })
        .unwrap();
    terminal
        .backend()
...
fn a_narrow_terminal_keeps_the_transcript_whole() {
    ...
    let narrow = app.sessions_panes(Rect::new(0, 0, 80, 30), &selection);
...
fn the_pane_stays_shut_on_a_selection_with_nothing_to_show() {
    ...
    let wide = app.sessions_panes(Rect::new(0, 0, 160, 40), &selection);
