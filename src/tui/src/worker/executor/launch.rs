//! Session spawn: launch a fresh harness on the blocking pool and build the
//! environment it spawns with.
//!
//! Whether a task reuses an idle session, launches fresh, or queues is decided
//! up front in [`super::probe`]; this module owns the *fresh-launch* half —
//! [`super::PtySessionExecutor::launch`] opens the harness through
//! [`PtyManager`](crate::worker::pty::PtyManager), and
//! [`super::PtySessionExecutor::spawn_env`] builds the environment and extra
//! arguments it spawns with.

use std::collections::HashMap;

use medulla::daemon::providers::RunTaskOptions;

use super::super::pty::LaunchSpec;
use super::types::{OpenedSession, PtySessionExecutor};
