...
//!    screen when it is asking. Matched on squashed text (see [`squash`]), so a
//!    match does not depend on where a full-screen TUI chose to put its spaces
//!    or how it wrapped the line.
...
//! the screen: the **terminal bell**. All three harnesses ring it when a turn
//! ends or a prompt opens. It is the vaguest cue and the most universal one, so
//! it is kept as the fallback that loses to anything named
