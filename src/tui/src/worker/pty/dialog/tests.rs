//! Tests for the parent dialog module — recognising a harness blocked on a startup
//! dialog.
//!
...
fn the_workspace_trust_dialog_is_recognised() {
    let dialog = super::blocking_dialog(CLAUDE_TRUST_DIALOG)
...
fn a_dialog_is_recognised_however_the_harness_laid_it_out() {
    ...
    assert!(super::blocking_dialog(&squeezed).is_some());
    let shouty = CLAUDE_TRUST_DIALOG.to_uppercase();
    assert!(super::blocking_dialog(&shouty).is_some());
}
...
fn the_bypass_permissions_disclaimer_is_recognised() {
    let dialog = super::blocking_dialog(CLAUDE_BYPASS_DISCLAIMER)
...
fn the_two_dialogs_are_told_apart() {
    ...
    let trust = super::blocking_dialog(CLAUDE_TRUST_DIALOG).unwrap();
    let bypass = super::blocking_dialog(CLAUDE_BYPASS_DISCLAIMER).unwrap();
...
fn claudes_modals_are_report_only() {
    ...
    for screen in [CLAUDE_TRUST_DIALOG, CLAUDE_BYPASS_DISCLAIMER] {
        let dialog = super::blocking_dialog(screen).unwrap();
        assert!(
...
fn codexs_trust_dialog_is_recognised_and_dismissed_with_enter() {
    let dialog = super::blocking_dialog(CODEX_TRUST_DIALOG)
...
fn codexs_trust_dialog_is_told_apart_from_claudes() {
    ...
    let codex = super::blocking_dialog(CODEX_TRUST_DIALOG).unwrap();
    let claude = super::blocking_dialog(CLAUDE_TRUST_DIALOG).unwrap();
...
fn codexs_interactive_update_prompt_is_always_skipped() {
    let dialog = super::blocking_dialog(CODEX_UPDATE_NOTICE)
...
fn the_passive_update_banner_is_not_treated_as_a_dialog() {
    ...
    assert_eq!(super::blocking_dialog(banner), None);
}
