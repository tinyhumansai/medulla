//! Core-runtime lifecycle branches: existing-thread adoption + snapshot seeding,
//! message folding, submit/abort routing, steering and fleet RPCs, session
//! create / fork / resume, context inspection, worker updates, and the
//! connection-drop / stall-watchdog transitions plus the `resync.required`
//! error-carried snapshot.
