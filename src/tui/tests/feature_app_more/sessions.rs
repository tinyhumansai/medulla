...
#[test]
fn agents_x_on_a_row_that_is_not_a_task_prompts_to_select_one() {
    let (mut app, _rt) = demo_app();
    tab(&mut app, "Sessions");
    // An empty rail is the simplest row-that-is-not-a-task: the demo fixture's
    // only row is the dispatched session, so dropping the events that folded it
    // leaves the cursor on nothing.
    app.snapshot.events.clear();
    assert!(app.selected_task_id().is_none(), "no task under the cursor");
...
fn agents_absent_harness_status_renders_nothing_extra() {
    ...
    tab(&mut app, "Sessions");
    // Narrow on purpose: the work panel writes its own `tasks · N/M done`
    // heading, and the cursor now lands on a task row that has one. Below the
    // panel's width threshold the only thing that could print `tasks ·` is the
    // harness board this test is about.
