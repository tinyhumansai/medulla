...
/// The medulla home directory: `MEDULLA_HOME` when set (and non-empty), else
/// `<home>/.medulla`.
pub fn medulla_home(env: &HashMap<String, String>, home_dir: &Path) -> PathBuf {
    match env.get("MEDULLA_HOME") {
        Some(path) if !path.is_empty() => PathBuf::from(path),
        _ => home_dir.join(".medulla"),
    }
}

/// The worker-profile file path: `<medulla-home>/worker.json`.
pub fn profile_path(env: &HashMap<String, String>, home_dir: &Path) -> PathBuf {
    medulla_home(env, home_dir).join("worker.json")
}
...
mod tests {
    ...
    #[test]
    fn medulla_home_prefers_env_then_default() {
        let e = env(&[("MEDULLA_HOME", "/custom/home")]);
        assert_eq!(
            medulla_home(&e, Path::new("/home/me")),
            PathBuf::from("/custom/home")
        );
        assert_eq!(
            medulla_home(&env(&[]), Path::new("/home/me")),
            PathBuf::from("/home/me/.medulla")
        );
        // Empty override is ignored.
        assert_eq!(
            medulla_home(&env(&[("MEDULLA_HOME", "")]), Path::new("/home/me")),
            PathBuf::from("/home/me/.medulla")
        );
    }
    ...
    fn profile_path_is_worker_json_under_home() {
        let p = profile_path(&env(&[]), Path::new("/home/me"));
